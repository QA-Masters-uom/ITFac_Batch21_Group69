...
export class PlantsPage extends BasePage {
  ...
  async gotoManagePlants() {
    await this.page.goto(`${config.uiBaseUrl}/ui/plants`);
    await this.page.waitForLoadState("networkidle");
  }

  private addPlantBtn = this.page.locator("text=Add a Plant");
  private confirmDeleteBtn = this.page.getByRole("button", {
    name: /confirm/i,
  });
  ...
  private errorToast = this.page.locator(
    ".toast-error, .alert-danger, text=/already exists|duplicate|error/i",
  );
  ...
  async addPlant(data: PlantForm) {
    ...
    await this.categorySelect
      .selectOption({ label: data.category })
      .catch(async () => {
  ...
  async deletePlant(name: string) {
    ...
    await this.page.waitForLoadState("networkidle");
    await expect(this.page.locator("tbody tr", { hasText: name })).toHaveCount(
      0,
      { timeout: 15000 },
    );
  }
