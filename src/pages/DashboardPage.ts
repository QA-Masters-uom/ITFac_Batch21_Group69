...
export class DashboardPage extends BasePage {
    ...
    async isCardVisible(cardName: string): Promise<boolean> {
        ...
        try {
            const cardLocator = this.page.locator(`.dashboard-card h6:has-text("${cardName}")`);
    ...
    async isLowStockIndicatorVisible(): Promise<boolean> {
        ...
        try {
            const lowStockLocator = this.page.locator('.dashboard-card:has-text("Plants") text=Low Stock');
    ...
    async hasRevenueField(): Promise<boolean> {
        ...
        try {
            const revenueLocator = this.page.locator('.dashboard-card:has-text("Sales") text=Revenue, .dashboard-card:has-text("Sales") :text("Rs")');
    ...
    async getRevenueValue(): Promise<string> {
        ...
        const revenueText = await salesCard.locator('text=/Rs|Revenue/').first().textContent();
    ...
    async hasSubCategoryCount(): Promise<boolean> {
        ...
        try {
            const categoriesCard = this.page.locator('.dashboard-card:has-text("Categories")');
