...
Then('the response should contain plants list', async function (this: CustomWorld) {
    const responseBody = this.parameters['apiResponseBody'];
...
Then('the response should contain categories list', async function (this: CustomWorld) {
    const responseBody = this.parameters['apiResponseBody'];
...
When('I send a GET request to {string} without authentication', async function (this: CustomWorld, endpoint: string) {
    const response = await fetch(`http://localhost:8081${endpoint}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
...
Then('the response should contain health status', async function (this: CustomWorld) {
    const responseBody = this.parameters['apiResponseBody'];
...
When('I send a POST request to {string} with the token', async function (this: CustomWorld, endpoint: string) {
    const token = this.parameters['token']; // Use 'token' to match setup.steps.ts
...
Then('the token should be invalidated', async function (this: CustomWorld) {
    ...
    const testResponse = await fetch('http://localhost:8081/api/plants', {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
Then('the response should contain sales list', async function (this: CustomWorld) {
    const responseBody = this.parameters['apiResponseBody'];
