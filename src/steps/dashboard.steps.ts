...
Then(
  "I should see the following cards:",
  ...
  async function (this: CustomWorld, buttonText: string) {
    ...
    switch (buttonText) {
      case "Manage Categories":
        ...
        break;
      case "Manage Plants":
        await dashboard.clickManagePlants();
        break;
      case "View Sales":
        await dashboard.clickViewSales();
        break;
      case "Open Inventory":
        await dashboard.clickOpenInventory();
        break;
      default:
...
When("I return to dashboard", async function (this: CustomWorld) {
  await this.page!.goto(`${config.uiBaseUrl}${config.uiDashboard}`);
});

Then(
  "the dashboard should display accurate category count",
...
Then(
  "the dashboard should display accurate plant count",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "the dashboard should display accurate sales count",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "the dashboard should display accurate inventory count",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "{string} button should be visible",
  async function (this: CustomWorld, buttonText: string) {
    const dashboard = new DashboardPage(this.page!);
...
Given(
  "I attempt to login with {string} credentials",
...
Then(
  "the authentication response status should be {int}",
...
When(
  "I attempt to access dashboard without authentication",
  async function (this: CustomWorld) {
    const response = await this.page!.request.get(
      `${config.uiBaseUrl}${config.uiDashboard}`,
    );
...
When(
  "I attempt to get categories data without authentication",
  async function (this: CustomWorld) {
    const response = await this.page!.request.get(
      `${config.apiBaseUrl}${config.apiCategories}`,
    );
    this.parameters["response"] = response;
  },
);
...
When(
  "I request {string}",
  async function (this: CustomWorld, resource: string) {
    ...
    switch (resource) {
      ...
      case "Categories Summary":
        endpoint = "/api/categories/summary";
        break;
      case "Plants Summary":
...
Then(
  "the response should contain sales data",
  async function (this: CustomWorld) {
...
Then(
  "the response should contain an access denied error",
  async function (this: CustomWorld) {
    const response = this.parameters["response"];
...
Then(
  "the response should contain category count data",
  async function (this: CustomWorld) {
    const response = this.parameters["response"];
...
Then(
  "the response should contain plant count data",
  async function (this: CustomWorld) {
    const response = this.parameters["response"];
    // Only validate if response was successful
    if (response.status() === 200) {
...
Then(
  "the response should contain inventory data",
  async function (this: CustomWorld) {
    const response = this.parameters["response"];
    const body = await response.json();
    expect(body).toBeDefined();
...
When(
  "I click on sidebar link {string}",
...
Given(
  "there are low stock plants in the system",
  async function (this: CustomWorld) {
...
Then(
  "I should see {string} indicator in the Plants card",
...
Then(
  "the {string} text should be visible",
...
When(
  ...
  async function (this: CustomWorld, pageName: string) {
    const baseUrl = `${config.uiBaseUrl}/ui`;
...
When(
  "I click on the App Title in the header",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "all sidebar links should navigate correctly",
  async function (this: CustomWorld) {
...
Then(
  "the Sales card should display {string} field",
...
Then(
  "the Revenue should show a currency value",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "the Revenue should be in {string} format",
  async function (this: CustomWorld, currencySymbol: string) {
    ...
    const hasRsSymbol =
      revenueValue.includes(currencySymbol) || revenueValue.includes("Rs");
...
Then(
  "the {string} button should be enabled for admin",
  async function (this: CustomWorld, buttonText: string) {
...
Then(
  "the Categories card should display {string} count",
  async function (this: CustomWorld, countType: string) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "the Sub category count should be visible",
  async function (this: CustomWorld) {
    const dashboard = new DashboardPage(this.page!);
...
Then(
  "the Sub category count should be numeric",
  async function (this: CustomWorld) {
